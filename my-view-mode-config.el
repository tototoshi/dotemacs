...
(defmacro my-make-view-mode-tmp-out-command (key key-name fn)
  `(progn (defun ,(my-view-mode-fn-name key-name) ()
            (interactive)
            (view-mode -1)
            (funcall ,fn)
...
(my-make-view-mode-tmp-out-command "x" "x" #'(lambda () (delete-char 1)))
(my-make-view-mode-tmp-out-command "X" "X" #'(lambda () (delete-backward-char 1)))
(my-make-view-mode-tmp-out-command "u" "u" #'undo)
